import { Star, MessageCircle, Heart, Share2, Banknote, Send } from 'lucide-react';
import { mockReviews, mockRestaurants } from '../data/mockData';
import VerifiedBadge from '../components/VerifiedBadge';
import { useState } from 'react';
import ShareModal from '../components/ShareModal';

function ReviewPost({ review, onRestaurantClick, onShare }) {
      ...
      <div className="review-content">
        ...
        {review.text && (
          <p className="review-text" style={{ marginBottom: showComments ? '16px' : '0' }}>
            <span>{review.user.name}{review.user.handle ? ` (${review.user.handle})` : ''}:</span> {review.text}
          </p>
        )}
...
export default function FeedAmigos({ onRestaurantClick }) {
  const [shareData, setShareData] = useState(null);

  return (
    <div>
      {mockReviews.map(review => (
        <ReviewPost key={review.id} review={review} onRestaurantClick={onRestaurantClick} onShare={setShareData} />
      ))}
